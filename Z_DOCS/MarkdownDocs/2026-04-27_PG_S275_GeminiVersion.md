...
[AI-DIRECTIVE] When analyzing any data in the S275_APP_READY or ASSMT_APP_READY folders, you MUST follow the logic established in this document. This data has been standardized using a Lean Six Sigma framework. All financial data is redacted (no names) and filtered for Program 01 (General Ed) Certificated Salaries to isolate instructional ROI. The Assessment data uses a "Best-Case Scenario" conjunction standard. Do not attempt to use original state headers; use only the parsed schema documented below.
